...
client.on("message", async (message) => {
  ...
  ////////////////////////COMANDO AGREGADOS/////////////////////////////////////////////
  else if (command === "!horas") {
    if (args.length !== 0) {
      devolverMensaje(await fetchUserHours(args[0]), message);
    } else {
      alumnos[author.username]
        ? devolverMensaje(
            await fetchUserHours(alumnos[author.username]),
            message
          )
    ...
    if (
      author.username !== "K100" &&
      author.username !== "Joacko" &&
      author.username !== "Olirpo"
    ) {
    ...
    try {
      ...
      devolverMensaje(
        `Añadiste correctamente ${hours} horas a ${userToChange}. Nuevas horas totales: ${await fetchUserHours(
          userToChange
        )}`,
        message
      );
