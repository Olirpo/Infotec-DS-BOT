...
const spreadsheetId = "14BAQXJdSQQDRVch_AxvKvGl7I9a-rHcwvCSH6YLnwes"; // NEW ID
...
const setUserHours = async (user, hours, date = today()) => {
  const range = "InfoTEC!A3:D102";
  let { requestedUserColumn, arrOfColumns } = await getUserColumns(user, range);
  const indexOfNewCell = requestedUserColumn.length;
  ...
  let res = null;
  try {
    res = await setSheetInfo("COLUMNS", range, arrOfColumns);
  } catch (error) {
    console.error(error);
  } finally {
...
const getSheetInfo = async (format, range) => {
...
const getUserColumns = async (user, range = "InfoTEC!A3:D103") => {
  const sheetData = await getSheetInfo("COLUMNS", range);

  const arrOfColumns = sheetData.data.values;

  const normalizedUser = toNormalForm(user);

  const requestedUserColumn = arrOfColumns
    .filter((e, index) => {
      if (!e[0]) return false;

      normalizedUserIterated = toNormalForm(e[0]);
      const requestedUserColumn =
        normalizedUserIterated.includes(normalizedUser);
